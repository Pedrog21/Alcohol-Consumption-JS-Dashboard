(function() {
  ...
  function perc2color(perc) {
      ...
      if(perc < 30) {
        b = 250;
        r = 0;
        g = Math.round((perc - 20) * 25);
      }
      else {
        if(perc < 40) {
          b = 250 - Math.round((perc-30) * 25 );
          r = 0;
          g = 250;
        }
        else {
          if(perc < 55) {
            ...
            if(perc < 70) {
              b = 0;
              r = 250;
              g = 250 - Math.round((perc-55) * 125/15);
            }
            else {
              if(perc < 85) {
                b = 0;
                r = 250;
                g = 125 - Math.round((perc-70) * 125/15);
              }
              else {
  		          b = 0;
  		          r = 250 - Math.round((perc-85) * 125/15);
                g = 0;
  	          }}}}}}
  ...
  var defaultOptions = {
    ...
    fills: {
      defaultFill: '#AAAAAA', //cor
      cornorte: perc2color(0.45*100),
      corcentro:perc2color(0.30*100),
      corlisboa:perc2color(0.25*100),
      coralentejo:perc2color(0.26*100),
      corfaro:perc2color(0.15*100)
    },
    data: {
      PTBR: { fillKey: 'cornorte' },
      PTAV: { fillKey: 'corcentro' },
      PTLI: { fillKey: 'corlisboa' },
      PTSE: { fillKey: 'coralentejo' },
      PTFA: { fillKey: 'corfaro' }
    },
    ...
    geographyConfig: {
        ...
        highlightFillColor: '#FC8D59', //Cor segunda
  ...
  */
  ...
  function addStyleBlock() {
    if ( d3.select('.datamaps-style-block').empty() ) {
      ...
      .html('.datamap path.datamaps-graticule { fill: none; stroke: #777; stroke-width: 0.5px; stroke-opacity: .5; pointer-events: none; } .datamap .labels {pointer-events: none;} .datamap path {stroke: #FFFFFF; stroke-width: 1px;} .datamaps-legend dt, .datamaps-legend dd { float: left; margin: 0 3px 0 0;} .datamaps-legend dd {width: 20px; margin-right: 6px; border-radius: 3px;} .datamaps-legend {padding-bottom: 20px; z-index: 1001; position: absolute; left: 4px; font-size: 12px; font-family: "Open Sans", sans-serif;} .datamaps-hoverover {display: none; font-family: "Open Sans", sans-serif; } .hoverinfo {padding: 4px; border-radius: 1px; background-color: #FFF; box-shadow: 1px 1px 5px #CCC; font-size: 12px; border: 1px solid #CCC; } .hoverinfo hr {border:1px dotted #CCC; }');
    }
  }
    ...
    this.svg.selectAll(".datamaps-subunit")
      .attr("data-foo", function(d) {
        ...
        layer.append("text")
          ...
          .style("font-family", options.fontFamily || "Open Sans")
